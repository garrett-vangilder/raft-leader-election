...
func (r *MemcachedReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	found := &corev1.Pod{}
	size := int(memcached.Spec.Size)
	err = r.Get(ctx, types.NamespacedName{Name: memcached.Name, Namespace: memcached.Namespace}, found)
	if err != nil && apierrors.IsNotFound(err) {
		for i := 0; i < size; i++ {
			dep, err := r.replicaSetForMemcached(memcached, i)
			if err != nil {
...
func (r *MemcachedReconciler) replicaSetForMemcached(
	memcached *cachev1alpha1.Memcached, podIndex int) (*corev1.Pod, error) {
	// ls := labelsForMemcached(memcached.Name)
	// replicas := memcached.Spec.Size
	image, err := imageForMemcached()
	...
	pod := &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "my-pod-" + strconv.Itoa(podIndex),
			Namespace: "default",
		},
		Spec: corev1.PodSpec{
			Containers: []corev1.Container{
